export const metaballShader = (bubbleCount) => `
    ...
    const int N_BUBBLES = ${bubbleCount};
    uniform float bubbleData[${bubbleCount * 4}];
    ...
    float map(vec3 p, vec3 c) {
      ...
      if (objType == 0) {
        d = sdSphere(sp, sphereR);
      } else if (objType == 1) {
        d = sdBox(sp, vec3(sphereR*0.8), 0.01);
      } else {
        d = sdRing(sp, vec2(sphereR*1.1, 0.015));
      }
      ...
      for (int i = 0; i < N_BUBBLES; i++) {
        ...
        d = smin(d, sdSphere(q - bPos, max(r, 0.001)), bubbleSmooth);
      }
    ...
    vec4 getSrc(vec2 uv) {
      ...
      return c.a > 0.01 ? vec4(c.rgb, 1.0) : bgColor;
    }
    ...
    void main() {
      ...
      if (hitCount > 0) {
        ...
        col *= mix(vec3(1), exp(-absorbColor * thickness), f2);
        col *= 1. + f2;
